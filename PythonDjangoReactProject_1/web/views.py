...
def food_list(request):
    ...
    return render(request, 'web/list.html', send_data)

def food_before(request):
    fno=request.GET['fno']
    response=redirect(f"/web/detail/?fno={fno}")
    response.set.cookie(f"web{fno}", str(fno))
    return response
...
def food_detail(request):
    ...
    return render(request, "web/detail.html", {"msg":"맛집 상세보기"})
